...
if not st.session_state.authenticated:
    ...
    if c2.button("Unlock System", use_container_width=True):
        if pwd == "stark":
...
<style>
    ...
    .ai-box { padding: 20px; border-left: 5px solid #00FFFF; background-color: #161B22; color: #E6EDF3; font-size: 16px; margin-bottom: 20px; font-style: italic;}
</style>
...
if os.path.exists(os.path.join(DATA_DIR, "lb.csv")):
    with col_a.expander("🏆 VIEW MASTER LEADERBOARD", expanded=False):
        st.dataframe(get_csv(os.path.join(DATA_DIR, "lb.csv")), use_container_width=True, hide_index=True)
if os.path.exists(os.path.join(DATA_DIR, "portfolio_weights.csv")):
    with col_b.expander("⚖️ PORTFOLIO OPTIMIZER (Risk Parity)", expanded=False):
        st.dataframe(get_csv(os.path.join(DATA_DIR, "portfolio_weights.csv")), use_container_width=True, hide_index=True)
...
if os.path.exists(os.path.join(DATA_DIR, "correlation_matrix.csv")):
    with st.expander("🕸️ Correlation Guardian (Asset Linkages)", expanded=False):
        st.dataframe(get_csv(os.path.join(DATA_DIR, "correlation_matrix.csv"), index_col=0).style.background_gradient(cmap='coolwarm', axis=None), use_container_width=True)
...
try:
    ...
    ticker = st.sidebar.selectbox("Select Asset to Analyze", [tk for cat in assets.values() for tk in cat])
except: 
    st.error("Assets file missing or invalid. Please check assets.json")
    st.stop()
...
if os.path.exists(os.path.join(dp, "pred.json")):
    ...
    if rep['Error'] > 3.0: 
        st.markdown(f"<div class='alert-box'>⚠️ AI HALLUCINATING: Deviation {rep['Error']:.2f}%. Predictions unstable.</div>", unsafe_allow_html=True)
    
    c_head1, c_head2 = st.columns([3, 1])
    c_head1.subheader(f"System Mode: {rep.get('Market_Mode', 'CALCULATING...')}")
    ...
    c1.metric("Current Price", f"₹{rep['Price']:,.2f}", delta=f"{rep['Error']:.1f}% Margin")
    ...
    st.markdown("### 🧠 AI Tactical Verdict")
    st.markdown(f"<div class='ai-box'>{rep.get('Verdict', 'Verdict computing...')}</div>", unsafe_allow_html=True)
    ...
    if os.path.exists(bt_file):
        ...
        fig_bt.add_trace(go.Scatter(x=bt_df['Date'], y=bt_df['Strategy_Equity'], name='AI Strategy', line=dict(color='#00FFFF')))
    ...
    matrix = {
        ...
        "1 Month": [f"₹{s['1_Month']['Extreme_Good']*inv:,.0f}", f"₹{s['1_Month']['Good']*inv:,.0f}", f"₹{s['1_Month']['Most_Likely']*inv:,.0f}", f"₹{s['1_Month']['OK']*inv:,.0f}", f"₹{s['1_Month']['Bad']*inv:,.0f}", f"₹{s['1_Month']['Extreme_Bad']*inv:,.0f}"],
    ...
    st.markdown("### 🌀 1,000-Path Monte Carlo Simulation")
    ...
    st.info("System is currently calculating predictions for this asset. Please wait for the hourly cycle to complete.")
